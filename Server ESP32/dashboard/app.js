...
(function () {
  ...
  async function commitCommands() {
    ...
    try {
      ...
      // 2. Prepare payload
      const mode = el.btnModeAuto.classList.contains('active') ? 'auto' : 'manual';
      const commands = {
        relay1: el.cmdRelay1.checked,
        relay2: el.cmdRelay2.checked,
      ...
      const payload = {
        message: `Dashboard: Update commands (${commands.relay1 ? 'R1:ON' : 'R1:OFF'}, ${commands.relay2 ? 'R2:ON' : 'R2:OFF'}, ${commands.led ? 'LED:ON' : 'LED:OFF'})`,
  ...
  function updateUIWithState(state) {
    ...
    if (state.telemetry) {
      ...
      if (last_seen) {
        ...
        // If seen within 120 seconds, mark as Online
        if (diffSeconds < 120) {
          setDeviceOnline(true);
        } else {
          setDeviceOnline(false, `ออฟไลน์ (ขาดการติดต่อไป ${formatTimeDiff(diffSeconds)})`);
        }
      } else {
        setDeviceOnline(false, 'ยังไม่เคยเชื่อมต่อ');
      }
    }
  ...
  function setDeviceOnline(isOnline, reason) {
    if (isOnline) {
      ...
      el.statusText.textContent = 'ESP32 ออนไลน์';
    } else {
