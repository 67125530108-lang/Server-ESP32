...
ไฟล์ `config.h` จะต้องอยู่ในโฟลเดอร์เดียวกับโค้ด `.ino` เสมอ (`esp32/esp32_github_client/config.h`) เพื่อให้ Arduino IDE คอมไพล์ได้และเปิดเป็นแท็บคู่กัน:

1. เข้าไปที่โฟลเดอร์ `esp32/esp32_github_client/`
2. ทำการคัดลอกไฟล์ `config.example.h` แล้วเปลี่ยนชื่อเป็น `config.h`:
   ```bash
   cp esp32/esp32_github_client/config.example.h esp32/esp32_github_client/config.h
   ```
